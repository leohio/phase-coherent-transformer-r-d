...
class ComplexRMSNorm(Module):
    ...
    def forward(self, x):
        # Manual complex L2 normalize: torch.norm on complex tensors is unsupported on MPS.
        denom = torch.sqrt((x.conj() * x).real.sum(dim=-1, keepdim=True).clamp_min(1e-12))
        return (x / denom) * self.gamma * self.scale

# feedforward with mod relu
# https://arxiv.org/abs/1511.06464v4

class ModReLU(Module):
    """Arjovsky 2016 modReLU: ReLU(|z| + b)^p · z/|z|.

    Phase-preserving: output's angle = input's angle (for ReLU(|z|+b) > 0).
    The original lucidrains port had `real + imag` which is NOT phase-preserving;
    fixed to `real * imag` here so the FFN preserves phase coherence across layers.
    """
    ...
    def forward(self, x):
        magnitude = F.relu(torch.abs(x) + self.bias) ** self.pow  # real, ≥ 0
        unit_phasor = torch.exp(1.j * torch.angle(x))              # complex, |·| = 1
        return magnitude * unit_phasor                              # phase-preserving
