...
- `PaperV4/LemmaA.lean` — Lemma A (M.7): the exact factorisation `P(ε) = R(φ̄) ∘ P(δ)` and the global-mode pass-through across an L1 stack. Machine-checked in full (no `sorry`); the quantitative `‖Ỹ_L − Y_L‖` bound is machine-checked as `lemmaA_bound` in `L2.lean` — exact, with no `O(φ̄²)` remainder (via the global estimate `|e^{iφ̄} − 1| ≤ |φ̄|`).
- `PaperV4/LemmaC.lean` — Lemma C (M.9, Doeblin contraction). Machine-checked in full (no `sorry`, 2026-08-05): the rank-1 Doeblin component annihilates the zero-mean subspace and the residual matrix has non-negative entries with row sums `1 − μ_D`, so the finite-dimensional `ℓ∞ → ℓ∞` bound follows by a direct big-operator argument — no coupling formalisation required.
- `PaperV4/L2.lean` — Definition 3, Definition 4, the quantitative Lemma A bound (`lemmaA_bound`), the M.6 step-4 geometric recurrence (`cascade_geometric_bound`), **`Theorem5Hypotheses`** (a `Prop` structure whose fields are exclusively standard mathematical conditions: per-layer L1.a, per-layer non-expansiveness, per-layer uniform zero-mean phase-Lipschitz stability, uniform output bound), and **`theorem5`** (Theorem 5 of M.10). `theorem5` is **proven without `sorry`**, with the depth-uniform constants *derived by induction* — no hypothesis is conclusion-shaped; `#print axioms PaperV4.theorem5` shows only the three standard Lean / Mathlib axioms `[propext, Classical.choice, Quot.sound]`. The hypothesis set is proven satisfiable (`theorem5Hypotheses_satisfiable`). Lemmas B (M.8) and D (M.10) are not re-proven in Lean against a concrete architecture — their single-layer conclusions are exactly the named standard hypotheses `zero_mean_phase_stable` / `substrate_nonexpansive`.

**Status summary (verified 2026-08-05).** Build is clean; **the project contains no `sorry` and no bespoke premises** (the earlier conclusion-shaped `Theorem5Premises.cascade_decomposition` field has been eliminated). Every hypothesis of every theorem is a community-standard condition (linearity, row-stochasticity, Doeblin minorisation, Lipschitz / non-expansiveness, uniform phase-Lipschitz stability, bounded range). Remaining Lean-side work — deriving the per-layer hypotheses for the concrete trained architecture — reduces to (i) Lemmas B and D (paper §M.5 classifies as "rigorous", to be Lean'd), and (ii) the M.11 closure (`Λ_S · sup_l ‖J_l|_{V_0}‖ < 1` preserved across layers + (S3) under training); this changes what is *instantiated*, not what is *assumed*.
