...
\begin{itemize}
  ...
  \item \texttt{PaperV4/LemmaA.lean} -- Lemma A (M.7): the exact factorisation $P(\varepsilon ) = R(\bar \varphi ) \circ  P(\delta )$ and the global-mode pass-through across an L1 stack. Machine-checked in full (no \texttt{sorry}); the quantitative $\|\tilde Y_L - Y_L\|$ bound is machine-checked as \texttt{lemmaA\_bound} in \texttt{L2.lean} -- exact, with no $O(\bar\varphi^2)$ remainder (via the global estimate $|e^{i\bar\varphi} - 1| \le |\bar\varphi|$).
  \item \texttt{PaperV4/LemmaC.lean} -- Lemma C (M.9, Doeblin contraction). Machine-checked in full (no \texttt{sorry}, 2026-08-05): the rank-1 Doeblin component annihilates the zero-mean subspace and the residual matrix has non-negative entries with row sums $1 - \mu_D$, so the finite-dimensional $\ell^\infty \to \ell^\infty$ bound follows by a direct big-operator argument -- no coupling formalisation required.
  \item \texttt{PaperV4/L2.lean} -- Definition 3, Definition 4, the quantitative Lemma A bound (\texttt{lemmaA\_bound}), the M.6 step-4 geometric recurrence (\texttt{cascade\_geometric\_bound}), \textbf{\texttt{Theorem5Hypotheses}} (a \texttt{Prop} structure whose fields are exclusively standard mathematical conditions: per-layer L1.a, per-layer non-expansiveness, per-layer uniform zero-mean phase-Lipschitz stability, uniform output bound), and \textbf{\texttt{theorem5}} (Theorem 5 of M.10). \texttt{theorem5} is \textbf{proven without \texttt{sorry}}, with the depth-uniform constants \emph{derived by induction} -- no hypothesis is conclusion-shaped; \texttt{\#print axioms PaperV4.theorem5} shows only the three standard Lean / Mathlib axioms \texttt{[propext, Classical.choice, Quot.sound]}. The hypothesis set is proven satisfiable (\texttt{theorem5Hypotheses\_satisfiable}). Lemmas B (M.8) and D (M.10) are not re-proven in Lean against a concrete architecture -- their single-layer conclusions are exactly the named standard hypotheses \texttt{zero\_mean\_phase\_stable} / \texttt{substrate\_nonexpansive}.
\end{itemize}

\textbf{Status summary (verified 2026-08-05).} Build is clean; \textbf{the project contains no \texttt{sorry} and no bespoke premises} (the earlier conclusion-shaped \texttt{Theorem5Premises.cascade\_decomposition} field has been eliminated). Every hypothesis of every theorem is a community-standard condition (linearity, row-stochasticity, Doeblin minorisation, Lipschitz / non-expansiveness, uniform phase-Lipschitz stability, bounded range). Remaining Lean-side work -- deriving the per-layer hypotheses for the concrete trained architecture -- reduces to (i) Lemmas B and D (paper \S{}M.5 classifies as ``rigorous'', to be Lean'd), and (ii) the M.11 closure ($\Lambda _S \cdot  \sup_l \|J_l|_{V_0}\| < 1$ preserved across layers + (S3) under training); this changes what is \emph{instantiated}, not what is \emph{assumed}.
