...
| **NIAH `L=2048` (mid)** | dim=256/L6, chunked attention, **`complex_sigmoid` N=3** (s=0/s=1/s=2 all confirmed 2026-05-09); other cells retracted (bugs/truncations) | `complex_sigmoid` (PCT) | **1.000** (eval_loss ~ 1e-4, deep) | [02_niah_L2048](02_niah_L2048/) |
...
- **Purely positional retrieval** (NIAH L=2048) → `complex_sigmoid` (PCT) deeply solves at N=3 (all seeds confirmed 2026-05-09); other cells pending clean re-run
...
4. **Vanilla `softmax` (real or complex) is structurally limited on long-range retrieval** — it cannot solve Copy d=2000 at any tested capacity. NIAH L=2048 cell-ranking for non-PCT cells is pending a clean re-run; what is currently confirmed is that **PCT (complex_sigmoid) deeply solves NIAH L=2048 at N=3** (all three seeds confirmed 2026-05-09) — i.e., a complex-disadvantaged purely positional task is solved by a complex-attention design.
