...
To check that the synthetic and algorithmic dominance pattern carries over to physical complex-domain data, we benchmark all six cells on the public RML2016 mirror of RadioML and on a 10-piece test split of MusicNet. Both are small-scale runs at \texttt{dim=64, depth=3} with N=2 seeds per cell (below the project's preferred N=3; noted as a limitation).
...
This appendix gives the definitions, theorem statements, and proof strategies for the two-level phase-coherence framework. Full proofs with explicit constants and a Lean machine-checked formalisation of Theorems 1 and 2 -- the latter from standard per-layer hypotheses, see M.5 below -- are in the companion document at \url{https://github.com/leohio/phase-coherent-transformer-r-d/tree/main/lean}.
...
\begin{itemize}
  ...
  \item \textbf{Lemmas B and D} are fully proven on paper (Lemma B's second-order remainder is bounded but not tightened; Lemma D is the standard transformer-stability argument). Not separately formalised in Lean; their single-layer conclusions appear as the standard per-layer hypotheses of \texttt{theorem5} (see below).
  \item \textbf{Lemma C} follows the standard Doeblin argument (Levin, Peres \& Wilmer 2017, Theorem 4.9) and is machine-checked in Lean (\texttt{lemmaC}, no \texttt{sorry}): in the finite-dimensional $\ell^\infty$ setting the rank-1 Doeblin component annihilates the zero-mean subspace and the residual matrix has row sums $1 - \mu_D$, so no coupling formalisation is needed. The Lean repository therefore contains no \texttt{sorry}.
  \item \textbf{Theorem 2 is machine-checked in Lean (\texttt{theorem5}, no \texttt{sorry}) from standard per-layer hypotheses only} (\texttt{Theorem5Hypotheses}). Every hypothesis is a community-standard mathematical condition -- none is bespoke, and none assumes the depth-uniform conclusion:
\end{itemize}

\begin{enumerate}
  \item Each layer is L1.a-coherent (provided by Theorem 1, machine-checked).
  \item Each layer is non-expansive in the $\ell^2$ norm (the conclusion of (S2) + Lemma D).
  \item Each layer is uniformly $K$-Lipschitz against zero-mean phase perturbations (the single-layer conclusion of Lemmas B + C under (S3)).
  \item The stack output is uniformly bounded by $Y_{\max}$ (the conclusion of (S1)).
\end{enumerate}

The $L$-independence of the cascade constants $(C_0, C_1) = (2K + Y_{\max}, 0)$ is \emph{derived} in Lean by induction on the layer stack (global mode passes exactly through the L1.a stack by Lemma A -- machine-checked, including the quantitative bound \texttt{lemmaA\_bound} with no $O(\bar\varphi^2)$ remainder -- and the zero-mean residual is absorbed by the entry layer and propagated through the non-expansive substrate). The geometric recurrence of step 4 in M.3 is separately machine-checked (\texttt{cascade\_geometric\_bound}), and the hypothesis set is machine-checked to be satisfiable (\texttt{theorem5Hypotheses\_satisfiable}), so the theorem is not vacuous.

\textbf{M.11 closure (the residual technical content of hypotheses 2--4).} Verifying hypotheses 2--4 for the concrete trained architecture from first principles requires two engineering verifications, both tractable and neither a deep open problem:
...
The companion appendix contains the full quantitative statements; the Lean repository's \texttt{AUDIT.md} records that \texttt{\#print axioms} on \texttt{PaperV4.theorem5} and \texttt{PaperV4.lemmaC} returns only the three standard Lean / Mathlib axioms \texttt{[propext, Classical.choice, Quot.sound]}.

\medskip

\section*{Appendix 0. List of tests conducted}
The following tests support the headline matrix and the per-hypothesis evidence in \S{}4 / \S{}5. Per-run raw records (per-step \texttt{metrics.jsonl}, \texttt{summary.json}, and \texttt{config.json} where retained) are published in the public data archive under \texttt{result/<family>/raw*/}; trained checkpoints are included where they were retained (depth-scaling sweep finals, Path-X best seed) -- checkpoints for most mid-scale sweep runs were not retained by the compute providers.
...
\textbf{Depth scaling sweep} -- \texttt{complex\_sigmoid} on LRA-ListOps L=1024 at depth $\in$ {2, 4, 6, 10, 14, 20}; H100, batch=32, lr=1e-3 cosine over 30K steps, single seed (N=1; an N=3 replication with an n=2048 deterministic final eval is in progress). Detailed in \S{}4.7 / \S{}5.5.
...
The public data archive contains, per experiment family, a summary write-up plus per-run raw artifacts (\texttt{metrics.jsonl}, \texttt{summary.json}, \texttt{config.json} and container logs where retained; see each family's \texttt{raw*/README.md} for provenance, including compute-provider task ids):
...
\hline
4.1 & NIAH L=2048 (mid) & 6 cells, complex chunked & dim=256, L=6, chunk=256 & 32 (mb=16, ga=2) & 3e-4 & 1000 & 30\,000 & 3 (\texttt{PCT}) / 1 (others) \\
\hline
...
\hline
4.2 & NIAH L=2048 & 6 cells, complex chunked & dim=256, L=6 & 32 (mb=16, ga=2) & 3e-4 & 1000 & 30\,000 (or 60\,000 at b=16) & 3 (\texttt{PCT}) / 1 (others) \\
\hline
...
\begin{itemize}
  ...
  \item \textbf{Seed convention}: multi-seed runs use seeds $s \in \{0, 1, 2\}$; "stuck-seed" extension to $N$=10 (\S 3.6) uses $s \in \{0, 1, \ldots, 9\}$. Actual seed counts vary by experiment and are stated per row in the Appendix-3 table (several older runs are N=1 or N=2; replications in progress are marked).
  \item \textbf{Reproduction artefacts}: the public data archive (Appendix~1) publishes, per run where retained, the per-step \texttt{metrics.jsonl}, a \texttt{summary.json} with the final metric and elapsed wall-clock, and the \texttt{config.json}; provenance (compute-provider task ids) is documented in each family's \texttt{raw*/README.md}.
\end{itemize}

\medskip

\section*{Appendix 3. Contents of \texttt{result/summary.md}}
This appendix is adapted from the orientation document \texttt{result/summary.md} in the public data archive linked in Appendix 1, with per-row seed counts corrected to the actual collected N (2026-08 audit). The archive file is available at \url{https://github.com/leohio/phase-coherent-transformer-r-d/blob/main/result/summary.md}.

\subsection*{Cross-task headline matrix}
The headline finding for each task, with the best-performing cell shown. "PCT-fair" means $real\_dim = complex\_dim \times 1.41$, screening cells with softmask=OFF, batch=32; seed counts are stated per row.
...
\hline
NIAH \texttt{L=2048} (mid) & dim=256/L6, chunked attention; \texttt{PCT} N=3, other cells N=1 & \texttt{complex\_sigmoid} (\texttt{PCT}) & \textbf{1.000} \\
\hline
LRA-ListOps medium (small) & dim=64, N=1 & \texttt{complex\_sigmoid} & 0.7188 \\
\hline
LRA-ListOps \texttt{L=1024} & dim=128, N=3 & \texttt{complex\_sigmoid} & 0.854 \\
\hline
LRA-Text 4K & dim=128, \texttt{PCT}-fair; N=2 (\texttt{complex\_sigmoid}) / N=3 (\texttt{complex\_screen}) & \texttt{complex\_sigmoid} + \texttt{complex\_screen} (dual) & 1.000 / 1.000 (others $\leq$ 0.64; held-out test-split re-measurement in progress) \\
\hline
LRA-Image CIFAR & dim=128, N=3 (an earlier N=6 count double-counted duplicated runs), \texttt{PCT}-fair & \texttt{complex\_sigmoid} & 0.458 \\
\hline
...
\hline
MusicNet real (small) & dim=64, N=2 & \texttt{complex\_screen} (marginal) & 0.218 \\
\hline
RadioML synth L1 / L2 & dim=64, N=2 & \texttt{complex\_sigmoid} (L1) / \texttt{complex\_screen} (L2) & 0.598 / 0.637 \\
\hline
RadioML real L1 / L2 & dim=64, N=2 & \texttt{real\_screen} & 0.363 / 0.389 \\
\hline
...
\hline
Depth scaling & dim=128, batch=32, N=1 (N=3 replication in progress), d $\in$ {2, 4, 6, 10, 14, 20} & \texttt{complex\_sigmoid} trains at all depths & no depth-related accuracy collapse (single-seed observation) \\
\hline
Path-X & complex screening + PCR hybrid (\S{}6), N=3, held-out n=20\,000 & screen + phase-coherent recurrence & 92.71 $\pm$ 0.89 (best 93.50) \\
\hline
\end{tabularx}
\end{table}
